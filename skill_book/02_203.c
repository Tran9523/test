...
#define TAX_HOUSE 1
#define TAX_INTERIOR 10

void get_purchase_information(float *, float *, float *, float *);
void print_receipt(float, float, float, float, float, float, float, float, float);
...
int main(void)
{
    ...
    get_purchase_information(&house_width, &house_length, &price_squared_meter, &discount);

    tax_interior = price_interior * (TAX_INTERIOR / 100.0);
    price_interior_total = price_interior + tax_interior;

    price_house = (house_width * house_length) * price_squared_meter;
    ...
    print_receipt(house_width, house_length, price_interior, price_house,
        tax_interior, tax_house, subtotal, discount, total);

    return 0;
}

void get_purchase_information(float *house_width, float *house_length,
    float *price_squared_meter, float *discount)        // call by reference
{
    printf("Width of house (m) : ");
    scanf("%f", house_width);
    printf("Length of house (m) : ");
...
float tax_house, float subtotal, float discount, float total)   //  call by value
{
    printf("\n\n--HOUSE INFORMATION--\n");
    printf("Width : %.2f\n", house_width);
    printf("Length : %.2f\n", house_length);
